...
+ Пользователи  
  POST(/finserver/auth/sign-up) - регистрация пользователя, в теле запроса передать json  
  POST(/finserver/auth/sign-in) - авторизация пользователя, в теле запроса передается json с логином и паролем, в ответ возвращается токен  
  GET(/finserver/user) - получить информацию о пользователе, в заголовке нужно указать токен пользователя  
  PACTH(/finserver/user) - обновить пользователя, в теле запроса передать json  
  DELETE(/finserver/user) - удалить пользователя, в заголовке необходимо передать токен удаляемого пользователя  
