...
Нужно еще подумать, как отображать остатки по картам и добавить учёт налички. А еще в конце сделать общий остаток. У меня же в таблице excel есть только одно поле с остатком, можно пока в программе сделать одно поле.  Также должно быть поле - накопительный счет, но я думаю тут не нужно делать отдельное поле, можно допустим сделать объект кошелек, который будет прикреплен к пользователю, в кошельке будут поля со счетами и поле user.